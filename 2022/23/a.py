...
min_y = min(elves, key=lambda x: x.get_y()).get_y()
max_y = max(elves, key=lambda x: x.get_y()).get_y()

min_x = min(elves, key=lambda x: x.get_x()).get_x()
max_x = max(elves, key=lambda x: x.get_x()).get_x()

tot = (max_y-min_y+1) * (max_x-min_x+1)
print(tot)
print(tot-len(elves))