...
def find_best(bp:tuple[int], res:tuple[int]=(0,0,0,0), prev_poss=(False, False, False), state:tuple[int,int,int,int]=(1,0,0,0), t:int=24):
	...
	if res[0] >= bp[5] and res[2] >= bp[6]: # always go here, prune rest
		...
		return find_best(bp, new_res, prev_poss, (state[0], state[1], state[2], state[3]+1), t-1)
	else:
		new_res = (res[0]+state[0], res[1]+state[1], res[2]+state[2], res[3]+state[3])
		nothing = find_best(bp, new_res, poss, state, t-1)
		if not prev_poss[0] and res[0] >= bp[1] and state[0] < (bp[2] + bp[3] + bp[5]): # ore
			...
			buy_ore = find_best(bp, new_res, (False, False, False), (state[0]+1, state[1], state[2], state[3]), t-1)
		if not prev_poss[1] and res[0] >= bp[2] and state[2] < bp[4]: # clay
			...
			buy_clay = find_best(bp, new_res, (False, False, False), (state[0], state[1]+1, state[2], state[3]), t-1)
		if not prev_poss[2] and res[0] >= bp[3] and res[1] >= bp[4] and state[3] < bp[6]: # obs
			...
			buy_obs = find_best(bp, new_res, (False, False, False), (state[0], state[1], state[2]+1, state[3]), t-1)
