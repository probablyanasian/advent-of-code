...
@cache
def find_w_ele(left:frozenset):
	max = 0
	for i in range(1, len(left)//2): # can div 2 b/c works WLOG
		for comb in itertools.combinations(left, i):
			pressure = find_best(nodes['AA'], 27, frozenset(comb)) + find_best(nodes['AA'], 27, left-set(comb))
			if pressure >= max:
				max = pressure
	return max
#31 to account fof AA node traversal time
print(find_w_ele(frozenset([node.id for node in nodes.values() if node.flowrate != 0])))